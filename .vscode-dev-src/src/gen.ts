...
function createWatcher(paths: string | string[], callback: () => Promise<any>) {
  ...
  watcher.on("all", async (event, pathA) => {
    if (!ready || ["addDir", "unlinkDir"].includes(event)) {
      return;
    }
    logger.debug(event, pathA);
...
async function genNls(opts: CLIOptions) {
  ...
  const defaultLocale = {
    ...readJsonSync(path.join(localePath, `${opts.lang}.json`)),
  };
  ...
  await Promise.all(
    files.map(async (name) => {
      ...
      const messages = { ...(await readJson(path.join(localePath, name))) };
