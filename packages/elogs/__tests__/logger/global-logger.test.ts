...
describe("globalLogger", () => {
  ...
  describe("top-level pino export", () => {
    test("pino is exported and is the underlying logger's pino", () => {
      // 顶层 `pino` 导出应已初始化,且与底层 Logger 的 pino 是同一引用
      // —— `globalLogger` **不**暴露 pino,这是有意的(避免 API surface 重复)
      expect(pino).toBeDefined();
      // 不能直接读 globalLogger.pino(globalLogger 接口不暴露 pino),
      // 通过 info 不抛错 + 行为一致来验证顶层 pino 走得通
