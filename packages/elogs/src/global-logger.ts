 ...
 *
 * - 模块初始化时(`createElogs()` 未被调用前)`pino` 已就绪,直接可用
 * - 后续 `initGlobalLogger()` / `createElogs()` 会用配置过的 logger 覆盖
 *   这个占位实例,确保所有路径(`globalLogger` + 顶层 `pino`)走同一份配置
 * - 若初始化失败(例如 `pino` 包没装上)则回退到 `undefined`,调用方需自行判空
 *
 * @example
 * ```ts
...
export let pino: Pino = (() => {
  try {
    // 模块加载时立刻建一个 minimal logger,让顶层 `pino` 不会 undefined
    // —— 真正的 logger 会在 initGlobalLogger() / createElogs() 时被覆盖
    return createLogger({}, undefined, globalContextStore).pino;
  } catch {
    // pino 缺失或 createLogger 异常时,允许顶层 `pino` 为 undefined
