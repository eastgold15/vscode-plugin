...
/**
 * 读取 JSON 文件，文件不存在或内容非法时返回空对象。
 * nls 文件缺失是正常情况（用户可能只提供了默认语言），不应抛错中断扩展启动。
 */
function readJsonSync(filePath: string): Record<string, string> {
  try {
...
export function loadI18n(extensionPath: string, language?: string) {
  ...
  let name =
    lang === DEFAULT_LANGUAGE ? DEFAULT_NLS : `package.nls.${lang}.json`;
...
export interface NlsI18n {
  ...
  use: ((messages: Record<string, string>) => void) &
    ((extensionPath: string, language?: string) => void);
}
...
export class I18n implements NlsI18n {
  ...
  t(
    ...
    // 两种调用形态共用一套占位符替换：命名占位符 {name} 走对象，
    // 位置占位符 {0} 走剩余参数数组。数组用 string 下标索引在 TS 里
    // 不合法，故统一收敛成 Record<string, unknown>。
    ...
    return text.replace(
      /\{([^}]+)\}/g,
      (match, group: string) => (values[group] ?? match) as string
    );
  }
}
